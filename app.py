...
# Modify to ensure it returns a 2D numpy array
def winsorize_series_robust(df_or_series, limits=(0.01, 0.01)):
    ...
    # Handle cases where the series might be empty or have non-numeric data
    if series_to_winsorize.empty or not pd.api.types.is_numeric_dtype(series_to_winsorize):
         # Return an empty 2D array if input is empty, or handle non-numeric appropriately
         # For a single column transformer, returning an empty array of shape (n_samples, 1) is typical
         return np.empty((len(series_to_winsorize), 0)) if series_to_winsorize.empty else np.empty((0, 0)) # Adjust as needed


    winsorized_array = winsorize(series_to_winsorize, limits=limits)

    # The key change: reshape the output to be 2D
...
if final_pipeline is not None: # Proceed only if the pipeline is loaded successfully
    ...
    with input_col1:
        ...
        selected_holiday = st.selectbox("Is it a Public Holiday?", options=list(holiday_map.keys()), format_func=lambda x: holiday_map[0]) # Corrected format_func
    ...
    if st.button("Get Prediction", help="Click to see the predicted bike demand."):
        ...
        try:
            ...
            # Call .predict() on the loaded PyCaret Pipeline object
